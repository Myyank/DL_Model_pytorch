...
def cross_entropy_loss(logits: torch.Tensor, labels: torch.Tensor):
    ...
    logitex = torch.exp(logits)
    logitsum = torch.sum(logitex, dim=1, keepdim=True)
    logits = torch.log(logitex/logitsum)
    logits = logits[range(batch_size), labels]  # checks for correctly classified labels
    return - torch.sum(logits)/labels.shape[0]
